...
# 超大仓库场景：调整自动降级的字节预算，或完全关闭降级（见下方"超大仓库"）
python3 scripts/repomap_lite.py -o REPOMAP.md --full-detail-budget-bytes 5000000
python3 scripts/repomap_lite.py -o REPOMAP.md --force-full-detail
...
- **文件之间的静态依赖关系**（哪个文件 import/require/include 了哪个
  项目内部的其他文件）——这是每个文件 block 里 `<!-- deps: ... -->`
  这一行提供的能力，详见下方"输出格式"和"文件间依赖"两节。这个能力
  **有明确边界**：只覆盖字面量层面就能确定目标的静态依赖声明，不覆盖
  运行时才能确定指向谁的动态依赖（变量拼路径、条件编译分支选择、插件
  加载），后者只标注"这里存在但解析不了"，不猜测具体目标。

它**回答不了**、需要另外读代码或读文档才能回答的问题：

- **调用关系**（"这个函数被谁调用""这条链路怎么串起来的"）——这是跟
  上面"文件间依赖"完全不同难度的问题：import/require/include 这类
  语句语法规整，正则能可靠识别；函数调用点分散在任意代码位置、需要
  跨文件建立符号表才能追踪，这已经越过了正则能可靠做到的范围，本工具
  刻意不做，避免做出一个不可靠的假调用图（一个内容有误的"关系图"比
  没有图更危险，人会更倾向于直接相信图上的箭头指向）
...
REPOMAP 建立"现在有什么、在哪、互相怎么引用"这层坐标，需要深入理解
某个具体机制或调用链路时再去读那几个关键文件的实际内容——不要指望仅凭
REPOMAP 就能回答"这个系统是怎么运转的"这类问题，那超出了这个工具的
设计范围，需要真正去读代码或者用更重的工具（完整的静态分析/调用图
生成，那是另一类工具要做的事，不建议往这个"零依赖、跨语言、秒级生成"
的工具里塞，会牺牲掉它现在最大的优点）。

## 文件间依赖：`<!-- deps: ... -->`

除了"这个文件里有什么"，每个文件 block 还会展示"这个文件依赖谁"——
用户提出的具体问题是"目前看起来就是罗列文件，没有文件间的关系，能不能
把关系表达出来"，这一节回答的正是这个问题。

**只做文件级静态依赖，不做调用关系/知识图谱**。经过逐语言分析后确定
的范围边界：`import`/`require`/`#include`/`use` 这类依赖声明语法在
所有已支持语言里都足够规整、值得正则可靠识别；"谁调用了谁"这类调用
关系需要跨文件符号表和数据流分析，跟"函数体逻辑"是同一个难度级别，
本工具不做（原因同上一节）。知识图谱式的实体关系存储+查询接口也不做
——那需要配套的存储和查询能力，已经不是"一份给 agent 读一次的文本
文件"能承载的东西，会变成另一个产品。
...
| 分类 | 含义 | 展示方式 |
|---|---|---|
| `internal` | 目标明确，确定是项目内部的另一个文件/模块 | 逐条列出目标路径（数量不超过8个时） |
| `external` | 目标明确，确定是第三方包/系统库/标准库 | 只给数量，不逐条列出 |
| `unknown` | 目标明确，但内部/外部归类信息不足，不确定 | 只给数量 |
| `dynamic` | 检测到依赖声明，但目标是运行时变量，无法静态解析 | 只给数量，`target` 为空 |

`internal` 是这份地图能提供的、源码之外查不到的真实增量信息（文件之间
怎么互相引用），所以逐条列出；`external` 往往是查一下 `package.json`/
`go.mod`/`Cargo.toml` 就知道的事实，只给数量避免占用篇幅（真实案例：
Redis 的 `src/server.c` 有47条 `#include`，如果逐条列出会让这一行本身
比合理长度长得多）。`unknown`/`dynamic` 如实标注"这里有信息，但解析
不出确定结论"，不是完全不提也不是编一个可能错的答案——这是跟上一轮
修复的字符串展示 bug 同一条原则的延伸："宁可展示不完整但诚实的结果，
也不要假装没有、或者给一个可能错的答案"。

各语言具体判断依据（哪些能确定 internal/external、哪些只能归 unknown，
以及每条规则的真实验证过程）见 `references/known_limitations.md`。
...
真实的 block 头部通常还带一行符号数注释和（如果这个文件有依赖声明的话）
一行依赖概览，两者都是机器可读注释，agent 可以直接忽略，也可以按需读取：

```
src/handler.py:
<!-- symbols: 3 -->
<!-- deps: internal=2(./config, ./utils) external=5 -->
⋮
...
`│` 后的定义行是**原始源码行原样展示**（含行尾注释，如
`│const REPO_ROOT = path.resolve(SITE_ROOT, '..') // git 仓库根（site/ 上一级）`）。
这是刻意选择：符号行忠实反映源码里的完整信息（字符串字面量、默认参数、
行尾注释都保留），让 agent 从地图上就能看到该行真正写了什么，而不是一个
被清洗过、可能误导的版本。代价是行尾注释偶尔会让符号行偏长，但对冷启动
定向这是信息量大于噪音的取舍。
...
## 超大仓库：自动降级为"索引优先"，避免几MB到几十MB的单文件输出

真实测过的极端场景：TypeScript 编译器这种数万文件级的仓库，完整展开
输出到过 13.8MB/44万行——agent 如果直接把这么大的文件整个读进上下文，
要么读取工具本身因为文件过大失败/截断（拿到一份不完整但自己不知道的
内容），要么读取"成功"但几乎耗尽整个对话的可用上下文，只为换来一份
没人会真正逐条查看的超长符号列表。

默认行为：完整展开的逐符号内容总量超过 2MB 预算后，**后续文件仍然
出现在顶部索引里**（带真实符号数，不会被静默漏掉），但不再展开完整
block，索引之后会有一行提示说明还剩多少文件/符号未展开，以及怎么单独
拿到它们的完整内容：

```
<!-- 已展开约 2048KB 内容（预算 2048KB），剩余 4407 个文件
（共 44070 个符号）只出现在上方索引里，未展开完整内容。
针对具体文件单独获取完整细节：--update-file <path>；
或用 --root 缩小扫描范围重新生成；
或用 --full-detail-budget-bytes 调大预算；
或用 --force-full-detail 完全关闭这个限制。 -->
```

索引本身也有独立的展示上限（默认最多 500 个文件，按符号数从多到少
排序），原因是真实测过一万文件规模的仓库，索引段本身能长到二十多万
字符——即使已经不展开逐符号细节，这个规模也不再是"扫一眼"该有的开销。
可见索引只展示前 N 行 + 一行聚合提示；超出上限的文件**仍完整保留**在
文件内的机器可读存档段（`<!-- INDEX-ARCHIVE-BEGIN ... INDEX-ARCHIVE-END -->`
注释块里，每条 `符号数\t路径`），增量更新靠它无损恢复这些文件的身份，
不会因为"索引展示截断"而静默丢文件。
...
# 调整索引展示的文件数上限（默认 500，传 0 表示不设上限）
python3 scripts/repomap_lite.py -o REPOMAP.md --index-max-entries 1000

# 完全关闭降级，不管仓库多大都展开全部内容（明确需要完整内容时用）
python3 scripts/repomap_lite.py -o REPOMAP.md --force-full-detail
```

这个预算用**渲染后的字节数**而不是符号数或文件数作为单位——开发过程中
踩过一次真实的坑：最初用符号总数做预算，理由是"符号数已经算好，更直接"，
但不同语言/代码风格的"平均每个符号占多少字节"差异很大（真实 Redis 语料
测出约62字节/符号，但换一种代码风格可以差好几倍），拿符号数做预算实际
控制的输出体积很不稳定，验证时曾经出现"预算内符号数达标，但实际输出仍
超过3MB"的情况——完全没有达到"把体积控制在安全范围"这个机制本该解决的
问题。改成直接量字节数之后，不管语言/风格如何变化，预算达到的效果都是
直接、可预期的：真实测过一个十万文件的合成测试仓库，默认预算下最终输出
稳定卡在约2.1MB，不会随着仓库规模线性膨胀下去。

`--update-file` 增量更新单个文件时不受这个预算影响——增量更新操作的是
"这一个文件"，不是重新决定整个仓库的展开范围，对单文件操作套用全仓库
预算于用户直觉不符，也偏离了"只是想更新这一个文件"的操作意图。

## 维护建议

- 单文件改动后，跑一次 `--update-file` 保持地图同步，比全量重扫快得多
- 大规模重构（批量增删文件）后，重新跑一次全量生成
- 改了脚本逻辑后，跑一次回归测试（覆盖预算降级、索引上限、增量更新保留
  被截断文件等行为，见 `scripts/tests/`）：
  ```bash
