...
## `stream: true` —— 走 SSE 流式，根治"等响应头超时"类 5xx

模型级可选字段，默认不配 = 非流式（一次 POST 等完整响应）。配了 `"stream": true` 的模型，
`openai_api` 适配器改成 SSE 流式：首包秒级到达，之后逐 chunk 拼装完整回复再进入下游解析，
末尾自动取带 `usage` 的 chunk 做 token 计量（请求体自带 `stream_options.include_usage`）。

**什么时候需要配**：请求经过的链路存在"等响应头"阶段的硬超时（典型：Cloudflare 边缘的
100s 源站超时，返回 524）时，非流式长任务（大图/长输出）必然被掐线，而流式因为首字节
秒级到达、之后只要 chunk 间隔不超限就可存活任意时长，能绕过这个窗口。判断标志：某模型
的大图/长输出任务稳定出现 `server_error 524`，而小任务正常——A/B 实测同任务非流式 524、
流式 200 且 101s 完整返回。

**配套约束**：

- `max_tokens` 勿给小值（如 ≤50）——gemma 系 `thinking=True` 会把小预算全吃掉，返回
  HTTP 200 + 空 `content`（另一个已实测的坑）；日常 300 起，长输出任务按需放大
- 配了 stream 的模型，`timeout` 仍按总时长给足（流式下是单 chunk 间隔超时，但保守起见
  别把 `timeout` 砍到比任务实际时长短）
- 保留 524 重试兜底逻辑不变：流式路径下再出现 524 才升级为故障排查（那意味着连首字节
  都没出来，属 origin 真异常）
